Drawable.Statemachine = function(_state_obj, target_paper, readonly, mode, active, locked) {
	...
	const state_class = paper.text(text_x, title_y_2,
	...
	const state_childs = paper.text(text_x, title_y_3, state_obj.getStates().length + " states")
		.attr({"text-anchor": 'start', fill: '#555', "font-size": font_size});
	...
	width += width_padding;
	height -= height_trim;
	const state_outer_box = paper.rect(0, 0, width, height);
	if (state_outer_box.insertBefore) state_outer_box.insertBefore(state_name);
	else state_outer_box.toBack();
	...
	const state_box = paper.rect(border_inset, border_inset, width - border_inset * 2, height - border_inset * 2);
	...
	if (locked) state_box
		...
		.attr({fill: '120-#eee:0-#fff:80', 'stroke-width': node_stroke_width});
