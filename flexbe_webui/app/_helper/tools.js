Tools = new (function() {
	...
	var pasteStateInto = function(s, sm, no_add) {
		...
		if (s instanceof Statemachine) {
			...
			new_state = new BehaviorState(s.getBehaviorName(), WS.Behaviorlib.getByKey(s.getStatePackage(), s.getBehaviorName()));
			new_state.setStateName(s.getStateName());
		} else if (s instanceof State) {
			const state_def = WS.Statelib.getFromLib(s.getStateType());
