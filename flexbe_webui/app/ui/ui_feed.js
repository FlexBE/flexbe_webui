UI.Feed = new (function() {
	...
	var displayVersionIndicator = function(latest_version_label) {
		...
		let status_element = document.getElementById("flexbe_version_status");
		...
		let current_version = current_version_label.split(".");
		let latest_version = latest_version_label.split(".");
	...
	this.displayCustomMessage = function(id, severity, title, content, child) {
		...
		let msg_table = document.createElement('table');
		...
		let msg_header = document.createElement('tr');
		...
		let msg_body = document.createElement('tr');
