UI.Dashboard = new (function() {
	...
	var createBehaviorParameterEdit = function(param_name) {
		...
		value_input_field.addEventListener("blur", function() {
			...
			var entry_value = this.value;
			this.style.backgroundColor = "#FFEBCD"; // blanched almond for primitives
			...
			if (entry.type == "boolean") {
				...
				this.style.backgroundColor = "#D5C5C"; // light coral for enum
			} else if (entry.type == "text") {
				this.value = this.value.replace(/^['"]+|['"]+$/g, '');
				if (this.value != entry_value){
					console.log(`stripped quote marks from parameter value ${name} value='${this.value}'`);
				}
				this.style.backgroundColor = "#7CFC00"; // lawn green for text
			} else {
				this.style.backgroundColor = "#FF4500"; // orange red
			}
	...
	this.behaviorPackageChanged = function() {
		...
		var new_package = selection_element.value;
