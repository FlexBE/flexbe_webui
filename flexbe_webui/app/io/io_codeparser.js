IO.CodeParser = new (function() {
	...
	var parseInitSection = function(code) {
		...
		if (comments_split.length > 1) {
			...
			var comments_lines = comments_code.split("\n");
			var start_comment = false;
			var end_comment = false;

			var start_parts = undefined;
			var comment_block = undefined;
			for (var i = 0; i < comments_lines.length; i++) {
				var line = comments_lines[i].trim();
				if (line.length == 0) {
					end_comment = true;
				}
				else if (line.startsWith("# !") || line.startsWith("# 0")) {
					start_comment = true;
					end_comment = false; // for multiple blank lines
					start_parts = line.split(" ");
					comment_block = "";
				}
				else {
					comment_block += line.replace("#","").trim() + "\n";
				}
				...
				if (end_comment && start_comment) {
					...
					if (start_parts.length > 4) {
						comment_path += start_parts[4].trim();
					}
					comments.push({
						important: start_parts[1] == "!",
						pos_x: parseInt(start_parts[2]),
						pos_y: parseInt(start_parts[3]),
						container: comment_path,
						content: comment_block
					});
					start_parts = undefined;
					comment_block = "";
				}
	...
	var parseStateParams = function(params, state_type_imports) {
			...
			if (param_result[1] == "transitions") {
				...
				for (var j=0; j<transition_list.length; j++) {
					...
					if (transition_kv != undefined) {
						if(transition_kv.value.includes('#')){
							// FlexBE WebUI style custom transition curve parameters
							var attr = transition_kv.value.trim().split('#');
							var attr1 = attr[1].trim().split(' ');
							transitions.push({
								outcome: helper_removeQuotes(transition_kv.key),
								target: helper_removeQuotes(attr[0].trim()),
								x: attr1[0],
								y: attr1[1],
								beg_x: attr1[2],
								beg_y: attr1[3],
		...
		return {
			...
			transitions_from: transitions			// [outcome, target] or [outcome, target, x, y, beg_x, beg_y, end_x, end_y]
		};
	}
	...
	this.parseCode = function(code) {
		...
		if (code_class_split.length != 4) {
			console.log("Failed to split code: ");
			console.log(code_class_split);
			throw "behavior class content could not be separated";
		}
	...
	var helper_splitOnTopCommas = function(code) {
		...
		for (var i=0; i<code.length; i++) {
			...
			// Handling string delimiters
			if (stringDelimiter !== null) {
				if (code.substring(i, i + stringDelimiter.length) === stringDelimiter && code[i - 1] !== '\\') {
					i += stringDelimiter.length-1;  // Advance to end of delimiter
					stringDelimiter = null;  // End of string
					continue;
				}
			} else {
				...
				if (`'"`.includes(c)) {
					if (i > 0 && code[i-1] === '\\') {
						// prior escape character, so ignore this quote mark
					} else {
						if (code.substring(i, i + 3) === '"""' || code.substring(i, i + 3) === "'''") {
							stringDelimiter = code.substring(i, i + 3);
							i += 2; // Skip to end of delimiter
						} else {
							stringDelimiter = c;
						}
					}
			...
			if (stringDelimiter != null) continue;  // continue searching for end of the string

			// Track depth when not in a string
			if ("([{".includes(c)) {
				depth++;
			}
			if (")]}".includes(c)) {
			...
			if (inside_lambda_eqn) {
				if (depth == inside_lambda_depth && c == ',') {
					// comma terminates a lambda unless in a deeper [( pair )], but stays in section
					inside_lambda_eqn = false;
					inside_lambda = false;
					inside_lambda_depth = -1;
					depth -= 1;
				} else if (depth < inside_lambda_depth) {
					// we exited lambda due to )]} on main level
					inside_lambda_eqn = false;
					inside_lambda = false;
					inside_lambda_depth = -1;
					depth -= 1; //  which should also terminates section
				}
			}
