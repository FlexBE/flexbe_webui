			...
			for (var i=0; i<container_states.length; i++) {
				...
				for (var j=0; j<s_def.transitions_from.length; j++) {
					...
					if('x' in trans_def){
						...
						parseInt(trans_def.beg_x), parseInt(trans_def.beg_y), parseInt(trans_def.end_x), parseInt(trans_def.end_y));
					}
					else{
						trans = new Transition(state_from, state_to, trans_def.outcome, autonomy);
					}
